...
def set():
    ...
    FoodService.setStockChangeLog(model_food.id,int(stock)-int(before_stock), "后台修改")  # 操作库存信息表FoodStockChangeLog
    return jsonify(resp)
...
def cat():
    ...
    list=query.order_by(FoodCat.weight.desc(),FoodCat.id.desc()).all()      #全集
    resp_data['list']=list
    resp_data['search_con']=req
    resp_data['status_mapping'] = app.config['STATUS_MAPPING']
...
def catSet():
    ...
    food_cat_info=FoodCat.query.filter_by(id=id).first()        #如果查询到分类信息则是 修改 操作，否则是 新增 操作
    if food_cat_info:
