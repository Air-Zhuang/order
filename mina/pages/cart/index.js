...
Page({
    ...
    jiaBtnTap: function (e) {
        ...
        this.setCart(list[parseInt(index)].food_id,list[parseInt(index)].number)        //触发setCart事件更新member_cart表
    },
    //减数量
    ...
    toPayOrder: function () {
        ...
        var list=this.data.list;            //选择当前被选中的商品，list为全局对象
        for(var i=0;i<list.length;i++){
            if(!list[i].active){
                continue;
            }
        ...
        wx.navigateTo({
            url: "/pages/order/index?data="+JSON.stringify(data)        //这里直接用get传参
        });
    },
    //如果没有显示去光光按钮事件
    ...
    setCart:function (food_id,number) {             //更新member_cart表信息
        var that=this;
        var data={
            "id":food_id,
            "number":number
